...
// 只循环一次，给原数组增加 num 属性
function addNum(arr, objLen) {
    if (!(arr instanceof Array) || !(objLen instanceof Object)) {
        return;
    }

    return arr.map(item => {
        if (item.zStock && objLen[item.zStock]) {
            item.num = objLen[item.zStock];
        }
        return item;
    });
}
...
// 获取等待时间及颜色
function getTimeAndColor(arr, obj) {
    if (!(arr instanceof Array) || !(obj instanceof Object)) {
        return;
    }

    return arr.map(item => {
       if (item.num && item.zStock) {
           if (obj[item.zStock] && Array.isArray(obj[item.zStock])) {
               obj[item.zStock].forEach(itemObj => {
                   if (item.num > itemObj.carNum) {
                       itemObj.color ? (item.color = itemObj.color) : null;
                       itemObj.time? (item.time = itemObj.time) : null;
                   }
               });
           }
       }
               ...
               if (obj[item.zStock][i + 1]) {
                   if (obj[item.zStock][i + 1].carNum) {
                       if (obj[item.zStock][i + 1].carNum > stockGroup[item.zStock]) {
                           item.color = obj[item.zStock][i + 1].color;
                           item.time = obj[item.zStock][i + 1].time;
                       }
                   }
               } else {
